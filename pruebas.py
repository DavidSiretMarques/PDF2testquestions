...
class TestSimu(QWidget):
    ...
    def end_test(self): #WIP
        ...
        for question in self._question_layout.children():
    ...
    def create_questions(self):
        ...
        self._question_layout = QVBoxLayout()
        question_group = QGroupBox()
        question_layout = QVBoxLayout()
        question_group.setLayout(question_layout)
        #Iteratively create question and options
        question_layout.addWidget(QLabel(self.questions[0]['pregunta'], wordWrap=True))
        [question_layout.addWidget(QRadioButton(opcion)) for opcion in self.questions[0]['opciones']]
        question_layout.addWidget(QFrame(frameShape=QFrame.HLine))
